...
class Category extends Model
{
    ...
    public function rooms(){
        return $this->hasMany('App\Models\Room','cat_id','id')->where('status','active');
    }
    public function sub_rooms(){
        return $this->hasMany('App\Models\Room','child_cat_id','id')->where('status','active');
    }
    public static function getRoomByCat($slug){
        // dd($slug);
        return Category::with('rooms')->where('slug',$slug)->first();
        // return Room::where('cat_id',$id)->where('child_cat_id',null)->paginate(10);
    }
    public static function getRoomBySubCat($slug){
        // return $slug;
        return Category::with('sub_rooms')->where('slug',$slug)->first();
    }
